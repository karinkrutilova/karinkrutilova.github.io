...
assert.match(orderPage, /grid-template-columns:\s*repeat\(4,/);
assert.match(orderPage, /@media \(width<=900px\)[^@]*grid-template-columns:\s*repeat\(3,/);
assert.match(orderPage, /@media \(width<=560px\)[^@]*grid-template-columns:\s*repeat\(2,/);
...
assert.match(globalStyles, /\.works::before\s*\{[^}]*inset:\s*0 calc\(50% - 50vw\)[^}]*background:\s*#0b0b0b/s);
assert.match(globalStyles, /\.works::after\s*\{[^}]*opacity:\s*\.35[^}]*background-image:\s*url\('\/gallery-smoke\.webp'\)/s);
