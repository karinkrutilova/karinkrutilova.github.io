...
assert.match(admin, /\?refresh=\$\{Date\.now\(\)\}/);
...
assert.equal([...orderPage.matchAll(/data-image-path="\/src\/assets\/works\/[^\"]+"/g)].length, artworkFiles.length);
const orderScript = await readFile('dist/admin/order.js', 'utf8');
assert.match(orderScript, /const repo = 'karinkrutilova\.github\.io'/);
assert.match(orderScript, /Reorder \$\{items\.length\} portfolio artworks/);
assert.match(orderScript, /createArtworkRecord/);
